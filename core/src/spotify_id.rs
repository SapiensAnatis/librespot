...
use crate::spotify_uri::SpotifyResourceName;
...
impl TryFrom<&SpotifyUri> for SpotifyId {
    ...
    fn try_from(src: &SpotifyUri) -> Result<Self, Self::Error> {
        match src.name {
            SpotifyResourceName::Id(id) => Ok(id),
            _ => Err(SpotifyIdError::InvalidFormat.into()),
        }
    }
}
